...
export function BentoCard({ item, className, index }: BentoCardProps) {
  ...
  const CardContent = (
    <div className="relative z-10 flex items-start justify-between">
      ...
      {item.stat && (
        <span
          className={cn(
            "font-bold font-mono tracking-tighter opacity-20 transition-opacity duration-300 group-hover:opacity-100",
  ...
  const CardBody = (
    <>
      ...
      <div
        className={cn(
          "absolute -top-12 -right-12 h-32 w-32 rounded-full opacity-20 blur-3xl transition-all duration-500 group-hover:scale-150",
  ...
  const containerClasses = cn(
    "group relative flex flex-col justify-between overflow-hidden rounded-3xl border-none p-6 text-left shadow-none ring-0 transition-colors duration-300",
    styles.card,
    item.className,
    className
  );
  ...
  if (item.detailList) {
    return (
      <Credenza>
        ...
        <CredenzaContent className="flex max-h-[80vh] flex-col sm:max-w-md">
          <CredenzaHeader>
